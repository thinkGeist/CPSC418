...
public class Server
{
	...
	if(args.length == 2 && args[1].compareTo("debug") == 0) {
		debugFlag = true;
	...
	try {
		...
		System.out.println("Common seed required, please provide your seed: ");
		String seedTxt = stdIn.readLine();
		if(debugFlag)
			System.out.println("Seed provided: " + seedTxt);
		 seed = seedTxt.getBytes();
	...
	while (!shutdown) {
		 ...
		 * run() methods. */
		st = new ServerThread (client, this, clientcounter++, debugFlag, seed);
